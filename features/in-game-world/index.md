...
进入游戏世界后的核心交互：地图探索、移动、与怪物/NPC/其他玩家互动。含玩家间交互（交易、组队）和现金商城入口。
...
- 命中特效（`SkillHitEffect`）、使用特效（`SkillUseEffect`）、音效（`SkillSound`）
- 伤害数字（`DamageNumber`）
...
- 支持多选项对话、商店（`UIShop`）
- 服务器驱动对话状态机（`NpcInteractionHandlers`）

## 玩家间交互

- **PlayerInteractionHandlers** — 处理玩家间交互协议（交易请求、组队邀请等）
- 相关数据包构建使用 `PlayerInteractionPackets.h`（**linux 分支特有**）

## 其他更新 vs mac 分支

- **UICharInfo** — 查看其他玩家角色详情
- **UICashShop** — 现金商城界面（通过 `UIStateCashShop` 管理）
...
- [agents/net/index.md](../../agents/net/index.md) — SetfieldHandlers、AttackHandlers、MapObjectHandlers、PlayerInteractionHandlers
- [agents/io/index.md](../../agents/io/index.md) — 键盘输入、UI 叠层（UIStateGame、UIStateCashShop）
