...
从游戏启动到进入游戏世界的完整认证与角色选择流程。Debug 模式下通过 `main()` 入口，Release 模式下通过 `WinMain()` 入口。
...
- 选择世界 → 选择频道
- 客户端发送世界/频道信息到服务器
- 支持自动登录（`get_auto_login()` 等配置项）

### 3. 登录认证（UILogin）
- 输入账号、密码、PIN（可选）
- 支持保存账号（`SaveLogin` 配置项）
- 支持自动登录：`get_auto_acc()`、`get_auto_pass()`、`get_auto_pic()`、`get_auto_cid()`、`get_auto_world()`、`get_auto_channel()`
- 认证成功后进入角色选择或角色创建界面
...
- **MISSING_FILE**: 缺少必要 NX 或 WZ 文件
- **WZ**: WZ 格式初始化失败（USE_NX 未定义时）
- **NLNX**: NX 格式初始化失败（USE_NX 定义时）
- 认证失败：账号密码错误，服务器返回错误消息，`LOG(LOG_ERROR, ...)` 输出
- 角色名重复：创建角色时名字已被占用
...
- [Configuration](../../Configuration.h) — ServerIP、ServerPort、SaveLogin、auto_login 等
