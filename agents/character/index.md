...
管理玩家角色状态：属性、外观、背包、技能、任务、经验。支持现金道具（Cash items）相关属性。
...
- `Pet` — 宠物
- `Weapon` — 武器类型定义
- `InventoryType` / `EquipStat` / `EquipQuality` — 类型与属性枚举
...
- `BodyDrawInfo` — 身体绘制参数
- `CharEquips` — 已装备物品外观管理
- `Clothing` / `Hair` / `Face` / `Body` — 各部位
...
- `CharEffect` — 角色特效
- `SkillBook` — 技能书（已学技能列表）
- `SkillId` — 技能 ID 定义
- `Job` — 职业枚举
- `ExpTable` — 经验表
- `QuestLog` — 任务日志
- `Buffs` / `ActiveBuffs` / `PassiveBuffs` — Buff 状态
- `MonsterBook` — 怪物图鉴
- `TeleportRock` — 传送石
- `StatCaps` — 属性上限定义
