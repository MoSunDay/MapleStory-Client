...
纹理资源，封装 NX/WZ 位图与 GraphicsGL 的交互。依据 `USE_NX` 宏，源数据来自 `nl::bitmap` 或 WZ 位图。

### Sprite（`Sprite.h/.cpp`）
精灵，与 Animation 配合从 NX/WZ 节点加载帧序列。

### Animation（`Animation.h/.cpp`）
帧动画，从 NX/WZ 节点加载，支持循环、延迟。
...
渲染模块与数据格式解耦：`Texture` 和 `Sprite` 封装了 NX/WZ 差异，上层统一调用 `draw()` 接口。当 `USE_NX` 未定义时，数据从 `WzFiles`（WZ 格式）加载。

## 依赖

- `includes/nlnx/` — NX 位图数据源（nl::bitmap）
- `Util/WzFiles.h` — WZ 位图数据源（USE_NX 未定义时）
- FreeType — 字体加载
- OpenGL / GLEW — 底层渲染
